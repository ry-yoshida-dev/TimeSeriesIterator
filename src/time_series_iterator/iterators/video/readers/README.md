...
|-----------|-------------|
| [torchcodec.py](./torchcodec.py) | `VideoBackend.TORCHCODEC` backend: decode via `torchcodec` on the configured `Device` (CUDA by default, using the GPU's NVDEC hardware instead of the CPU), a run of consecutive frames per decode call while iterating |
